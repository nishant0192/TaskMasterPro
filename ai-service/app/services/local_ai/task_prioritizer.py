...
class PersonalizedTaskPrioritizer:
    ...
    async def initialize(self):
        ...
        try:
            logger.info(f"Initializing task prioritizer for user {self.user_id}")
            ...
            logger.info(f"Task prioritizer initialized for user {self.user_id}")
    ...
    async def _train_initial_model(self):
        ...
        try:
            ...
            if len(training_data) < 10:
                logger.warning(f"Insufficient training data for user {self.user_id}, using synthetic data")
            ...
            async with get_async_session() as session:
                ...
                query = select(TaskEmbedding).where(
                    and_(
                        ...
                        TaskEmbedding.completion_status.in_(['completed', 'abandoned'])
                    )
    ...
    def _extract_features_from_task_record(self, record: TaskEmbedding) -> Dict[str, Any]:
        ...
        # Context features
        features['is_peak_hour'] = 1 if record.created_hour in self.completion_history.get('peak_hours', []) else 0
    ...
    async def _train_model(self, training_data: pd.DataFrame):
        ...
        try:
            ...
            feature_cols = [col for col in training_data.columns if col != 'priority_label']
            ...
            logger.info(f"Model trained for user {self.user_id} with accuracy: {accuracy:.3f}")
            
            # Save model
            await self.model_manager.save_user_model(
                self.user_id, 'prioritization', self.model,
                model_subtype='classification',
                metadata={'feature_columns': self.feature_columns, 'accuracy': accuracy},
            ...
            if len(features_df) > 0 and self.model:
                ...
                for i, task in enumerate(tasks):
                    ...
                    prioritized_task = PrioritizedTask(
                        **task.dict(),
                        ai_priority_score=float(priority_score) / 5.0,  # Normalize to 0-1
    ...
    async def _extract_task_features(self, tasks: List[TaskBase], 
        ...
        for task in tasks:
            ...
            features['category_encoded'] = category_mapping.get(task.category, 0)
            
            # Time-based features
            peak_hours = self.completion_history.get('peak_hours', [9, 10, 14, 15])
            ...
            if task.due_date:
                ...
                features['urgency_score'] = max(0, 1 - (hours_until_due / 168))  # 1 week = 168 hours
            else:
            ...
            # Context features
            features['calendar_busy'] = 1 if context.get('calendar_busy', False) else 0
    ...
    async def _calculate_priority_factors(self, task: TaskBase, features: pd.Series, 
        ...
        factors['energy_alignment'] = float(is_peak_hour * 0.5 + energy_level * 0.5)
        ...
        try:
            ...
            async with get_async_session() as session:
                ...
                interaction = UserInteractionLog(
                    ...
                    task_id=task_id,
                    before_state={'predicted_priority': prediction.predicted_value if prediction else None},
        ...
        try:
            async with get_async_session() as session:
                ...
                if len(recent_predictions) >= 10:  # Minimum feedback threshold
                    ...
                    avg_accuracy = np.mean([p.prediction_accuracy for p in recent_predictions])
                    
                    if avg_accuracy < 0.7:  # Accuracy threshold
                        logger.info(f"Triggering model retrain for user {self.user_id} (accuracy: {avg_accuracy:.3f})")
            ...
            async with get_async_session() as session:
                ...
                query = select(UserInteractionLog).where(
                    and_(
                        ...
                        UserInteractionLog.interaction_type.in_(['task_created', 'priority_changed']),
                ...
                if interactions:
                    preferences['prefers_high_priority'] = high_priority_count / len(interactions)
                    preferences['changes_priority_often'] = priority_changes / len(interactions)
    ...
    async def cleanup(self):
        ...
        self.completion_history.clear()"Failed to initialize task prioritizer: {e}")
            raise
